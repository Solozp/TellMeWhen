...
-- Major updates by Oozebull of Twisting Nether
...
TellMeWhen_BuffEquivalencies["Incapacitated"] ="Gouge;Maim;Repentance;Reckless Charge";
TellMeWhen_BuffEquivalencies["StunnedOrIncapacitated"] ="Gouge;Maim;Repentance;Reckless Charge;Bash;Pounce;Starfire Stun;Intimidation;Impact;Hammer of Justice;Stun;Blackout;Kidney Shot;Cheap Shot;Shadowfury;Intercept;Charge Stun;Concussion Blow;War Stomp";
		...
		if ( iconType == "cooldown" ) then
			if ( CooldownType == "spell" ) then
				if ( GetSpellCooldown(TellMeWhen_GetSpellNames(icon.Name,1)) ) then
					...
					TellMeWhen_Icon_ClearScripts(icon);
					--only use ? texture when old texture not already in use
...
function TellMeWhen_Icon_ReactiveCheck(icon)
	...
	if ( usable ) then --and TellMeWhen_GetGCD() < timeLeft ) then
		if( inrange ~= 0 and nomana ~= 1 ) then --and timeLeft == 0) then
			--icon.texture:SetVertexColor(1,1,1,icon.usableAlpha)
			icon:SetAlpha(icon.usableAlpha)
		elseif ( inrange ~= 1 or nomana == 1) then
			--icon.texture:SetVertexColor(.35,.35,.35,icon.usableAlpha)
			icon:SetAlpha(icon.usableAlpha)
		else
			--icon.texture:SetVertexColor(1,1,1,icon.usableAlpha)
		...
		--icon.texture:SetVertexColor(1,1,1,icon.unusableAlpha)
		icon:SetAlpha(icon.unusableAlpha)
	end
end
